...
if (
    ...
    req.query['hub.verify_token'] == 'pruebawhat'
) {
...
router.post("/webhook", function (req, res) {
    ...
    // console.log(req.query)
        ...
        var headers = {
            'User-Agent': 'Super Agent/0.0.1',
            'Content-Type': 'application/json',
            'Authorization': 'Bearer EAACp7K2iFcABAK2yQbHPlS6gdRr6leI7WijdggPoZB3ZBfaCS0yqvUohky1YcsYzsqhDZCexqAu3yu5RWD8PclUW6Ww1dUl7d6RZC4NYm8Q0hcW5P8KaC8LDshntmZC1BS7wX7bCgYECfH4ZAEC11d4RS5QDRDTcmhrNvPhlzc3UlACPOapAWv'
        }

        if (store.get(from).data.length >= 16) {
            if (store.get(from).paso != 6 && store.get(from).data[10].indexOf("image") > 0
            ) {
            peticioneswhatasapp(
                store.get(from).numero,
                "Estas enviando un tipo de dato que no corresponde a la Respuesta esperada por favor intenta nuevamente...😊",
                "Text"
            );
            return;
            }
            ...
            if (
            store.get(from).data[15].indexOf("Ir a departamentos.") >= 0 &&
            store.get(from).numero == from
            ) {
            oficinas();
            return;
            }
            if (
            store.get(from).data[15].indexOf("volver al inicio") >= 0 ||
            store.get(from).data[15].indexOf("Volver al inicio") >= 0 ||
            store.get(from).data[15].indexOf("inicio") >= 0 ||
            store.get(from).data[15].indexOf("Inicio") >= 0 ||
            store.get(from).data[15].indexOf("Iniciar") >= 0 ||
            store.get(from).data[15].indexOf("iniciar") >= 0 ||
            store.get(from).data[15].indexOf("Menú") >= 0 ||
            store.get(from).data[15].indexOf("menú") >= 0 ||
            store.get(from).data[15].indexOf("Menu") >= 0 ||
            store.get(from).data[15].indexOf("menu") >= 0
            ) {
            store.set(from, {
                numero: store.get(from).numero,
            ...
            if (
            store.get(from).data[10].indexOf("Salir") >= 0 || store.get(from).data[10].indexOf("salir") >= 0
            ) {
            peticioneswhatasapp(
                store.get(from).numero,
                "Fue un placer atenderte, gracias por utilizar nuestros servicios.\nIngresa a nuestra sucursal virtual 🖥️ aquí https://bit.ly/3mgcBQM,\ndescarga la app asociados 📱 para Android aquí https://bit.ly/3pXCshr\npara IOS aquí https://apple.co/3w2JTWS podrás informarte, consultar tus saldos y realizar transacciones, te esperamos de nuevo...😊",
            ...
            if (
            store.get(from).data[10].indexOf("volver al inicio") >= 0 ||
            store.get(from).data[10].indexOf("Volver al inicio") >= 0 ||
            store.get(from).data[10].indexOf("inicio") >= 0 ||
            store.get(from).data[10].indexOf("Inicio") >= 0 ||
            store.get(from).data[10].indexOf("Iniciar") >= 0 ||
            store.get(from).data[10].indexOf("iniciar") >= 0 ||
            store.get(from).data[10].indexOf("Menú") >= 0 ||
            store.get(from).data[10].indexOf("menú") >= 0 ||
            store.get(from).data[10].indexOf("Menu") >= 0 ||
            store.get(from).data[10].indexOf("menu") >= 0
            ) {
            store.set(from, {
                numero: store.get(from).numero,
    ...
    // }
        ...
        if (store.get(from).Primeravez == 'S') {
            ...
            var options1 = {
                url: 'https://graph.facebook.com/v16.0/115743754834725/messages',  // 'http://localhost:3021/send?message=' + message + '&to=' + number,
                method: 'POST',
                JSON: true,
                headers: headers,
                rejectUnauthorized: false,
